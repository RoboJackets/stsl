...
{
    if(!yaml.IsSequence()) {
        throw YAML::ParserException(yaml.Mark(), "std::array<double, 36> must be parsed from a sequence.");
    }

    if(yaml.size() != 36) {
        throw YAML::ParserException(yaml.Mark(), "Expected 36 elements in sequence, but got " + std::to_string(yaml.size()));
    }

    std::array<double, 36> msg;

    std::transform(yaml.begin(), yaml.end(), msg.begin(), [](const YAML::Node & n){
        return n.as<double>();
    });
