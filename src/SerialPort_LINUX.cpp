...
#include <cstring>
#include <termio.h>
#include "STSL/SerialPort_LINUX.h"
...
bool SerialPort_LINUX::Open(std::string device, unsigned int baud) {
...
void SerialPort_LINUX::Write(std::string message) {
    ssize_t n_written = 0;
    ssize_t spot = 0;
...
std::string SerialPort_LINUX::ReadLine() {
    string line;
    ssize_t n_read = 0;
    char in;
    do {
...
void SerialPort_LINUX::SetProperties(unsigned int baud) {
    struct termios tty;
